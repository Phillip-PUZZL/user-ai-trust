...
if not DATABASE_URL:
    raise RuntimeError("DATABASE_URL is not set. Example: "
...
def load_historical_figures():
    with open("./api/historical_figures.json", "r", encoding="utf-8") as file:
...
@app.route("/health")
def health():
    # Quick check: can we grab a connection?
    try:
...
if __name__ == "__main__":
    # In EB, Gunicorn will run the app; this is for local dev only.
